...
router.get('/', async (req, res) => {
  try {
    const { month, year, category, person, type, page = 1, limit = 50 } = req.query;
...
router.post('/', async (req, res) => {
  try {
    const { amount, description, type, person, date } = req.body;

    if (!amount || !description || !type || !person) {
      return res.status(400).json({ error: 'Campos obrigatórios: amount, description, type, person' });
    }

    const tx = await createTransaction({ amount: Number(amount), description, type, person, date, source: 'web' });
