...
const SK_COLAB   = 'creatorizando_colaboradoras';
const SK_PAGTOS  = 'creatorizando_colaboradoras_pagtos';
const SK_BONUS   = 'creatorizando_colaboradoras_bonus';

const EMPTY_COLAB = { name: '', amount: '', dayOfMonth: 5, pixKey: '', active: true };

export default function AgenciaColaboradoras() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-gray-900">👩‍💼 Colaboradoras</h2>
          <p className="text-sm text-gray-500">Pagamentos, bônus e controle mensal</p>
        </div>
              ...
              <div className="flex gap-2">
                {[['true', 'Ativa'], ['false', 'Inativa']].map(([v, l]) => (
                  <button key={v} type="button" onClick={() => set('active', v === 'true')}
                    className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${String(form.active) === v ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
                    {l}
                  </button>
                ))}
              </div>
        ...
        <div className="card border-2 border-yellow-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input className="input" placeholder="Ex: Bônus campanha X, meta batida..." value={bonusForm.descricao} onChange={e => setBonusForm(f => ({ ...f, descricao: e.target.value }))} />
            </div>
            ...
            {list.map(c => {
              ...
              return (
                <div key={c.id} className={`rounded-xl border p-3 transition-all ${pago ? 'bg-green-50 border-green-200' : atrasado ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-transparent'}`}>
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => togglePagamento(c.id)}
                        className={`text-xs px-3 py-1 rounded-full font-medium transition-colors ${pago ? 'bg-green-100 text-green-700 hover:bg-red-100 hover:text-red-600' : atrasado ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`}>
                  ...
                  {/* Bônus desta colaboradora no mês */}
                  {bonusColab.length > 0 && (
